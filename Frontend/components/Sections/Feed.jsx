    ...
    const fetchVideos = async () => {
      try {
        ...
        const response = await fetch(
          `https://youtube.googleapis.com/youtube/v3/videos?part=snippet%2CcontentDetails%2Cstatistics&chart=mostPopular&maxResults=30&regionCode=US&videoCategoryId=${category}&key=${data} `
        );
  ...
  return (
    <div className="Thumbnail-container">
      {videos && videos.length > 0 ? (
        videos.map((video, index) => <Videolayout key={index} video={video} />)
      ) : (
